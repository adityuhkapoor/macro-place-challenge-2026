...
class AdityaPlacerMultiV2:
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        for seed in self.seeds:
            # Perturb initial hard positions with seed-specific jitter
            rng = np.random.RandomState(seed)
            jitter = torch.from_numpy(
                rng.uniform(
                    -self.jitter_scale, self.jitter_scale,
                    size=(n_hard, 2)
                )
            ).float()
            jitter[:, 0] *= cw
            ...
            benchmark.macro_positions = perturbed
            ...
            if self.verbose:
                print(f"  [multi] seed={seed} cost={cost:.4f}")
            if cost < best_cost:
                best_cost = cost
                best_pos = pos.clone()
                best_seed = seed
        ...
        if self.verbose:
            print(f"  [multi] best seed={best_seed} cost={best_cost:.4f}")
        return best_pos
